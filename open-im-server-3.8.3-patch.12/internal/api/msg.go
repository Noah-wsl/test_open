...
const (
	// ScreenshotContentType 截图消息内容类型
	ScreenshotContentType = 115
)
...
func (m *MessageApi) getSendMsgReq(c *gin.Context, req apistruct.SendMsg) (sendMsgReq *msg.SendMsgReq, err error) {
	...
	case ScreenshotContentType:
		data = apistruct.ScreenshotElem{}
	default:
...
// SendScreenshotMessage 发送截图消息
func (m *MessageApi) SendScreenshotMessage(c *gin.Context) {
	req := apistruct.SendMsgReq{}
	if err := c.BindJSON(&req); err != nil {
		apiresp.GinError(c, errs.ErrArgs.WithDetail(err.Error()).Wrap())
		return
	...
	req.ContentType = ScreenshotContentType
	sendMsgReq, err := m.getSendMsgReq(c, req.SendMsg)
	if err != nil {
		apiresp.GinError(c, err)
		return
