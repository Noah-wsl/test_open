...
func newGinRouter(ctx context.Context, client discovery.SvcDiscoveryRegistry, config *Config, screenshotDB database.ScreenshotInterface) (*gin.Engine, error) {
	...
	// Screenshot 截图服务
	{
		sc := NewScreenshotApi(third.NewThirdClient(thirdConn), screenshotDB)
		screenshotGroup := r.Group("/screenshot")
		screenshotGroup.POST("/upload/init", sc.InitUpload)                     // 初始化上传
		screenshotGroup.POST("/upload/complete", sc.CompleteUpload)             // 完成上传
		screenshotGroup.POST("/list", sc.ListScreenshots)                       // 截图列表
		screenshotGroup.POST("/delete", sc.DeleteScreenshot)                    // 删除截图
		screenshotGroup.POST("/batch_delete", sc.BatchDeleteScreenshots)        // 批量删除
	}
	// Message
	{
		...
		msgGroup.POST("/send_screenshot_msg", m.SendScreenshotMessage)
