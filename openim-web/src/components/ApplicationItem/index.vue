<template>
  <div
    class="flex items-start border-b border-b-gap-text px-[22px] py-3 cursor-pointer md:hover:bg-[var(--primary-active)] md:transition-colors"
    @click="toDetails"
  >
    <Avatar
      :src="getIcon"
    ...
    <div class="relative ml-4 flex h-full w-full items-center justify-between">
      ...
      <div class="flex items-center">
        ...
        <van-button v-else size="small" hairline type="primary" @click.stop="toDetails">{{
          $t('buttons.toDetail')
        }}</van-button>
      </div>
    </div>
...
type ApplicationItemProps = {
  ...
  index?: number
  total?: number
  onToDetail?: (source: Partial<ApplicationItemSource>, type: ApplicationTypeEnum) => void
}
...
const toDetails = () => {
  if (window.innerWidth >= 768 && props.onToDetail) {
    props.onToDetail(props.source, props.type)
    return
  }
  if (!showActionBtn.value) return
  router.push({
