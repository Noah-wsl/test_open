...
export const setApprovalInstances = (list: any[]) =>
  localStorage.setItem(`${getIMUserID()}_approval_instances`, JSON.stringify(list))
export const getApprovalInstances = () => {
  const raw = localStorage.getItem(`${getIMUserID()}_approval_instances`)
