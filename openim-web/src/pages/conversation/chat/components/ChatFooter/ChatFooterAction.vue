<template>
  <div ref="target" class="bg-[#F0F2F6]">
    ...
    <van-uploader
      ...
      multiple
      max-count="1"
      :after-read="afterReadCameraFile"
    />
    <van-action-sheet
...
type ChatFooterActionEmits = {
  ...
  (event: 'getScreenshotFile', uploadData: UploaderFileListItem): void
  (event: 'startScreenshot'): void
}
...
const actionList = computed(() => {
  const baseActions = [
    ...
    },
    {
      ...
      icon: camera,
      type: ChatFooterActionType.Screenshot,
    },
  ]
  if (!isSingle.value) {
    return baseActions
...
const clickAction = ({ type }: ChatFooterActionItem) => {
  ...
  switch (type) {
    case ChatFooterActionType.Album:
      ...
      break
    case ChatFooterActionType.Screenshot:
      emit('closeActionBar')
      if (window.innerWidth >= 768) {
        emit('startScreenshot')
      } else {
        nextTick(() => cameraUploaderRef.value?.chooseFile())
      }
      break
...
const afterReadCameraFile = (data: UploaderFileListItem | UploaderFileListItem[]) => {
  if (!Array.isArray(data)) {
    data = [data]
  }
  data.map((item) => {
    emit('getScreenshotFile', item)
  })
}
</script>
